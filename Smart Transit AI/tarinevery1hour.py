...
while True:
    ...
    df["Holiday"] = df["Holiday"].replace({"Yes": 1, "No": 0}).fillna(0).astype(int)
    df["Special Event"] = df["Special Event"].replace({"Yes": 1, "No": 0}).fillna(0).astype(int)
